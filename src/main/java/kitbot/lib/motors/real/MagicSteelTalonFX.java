...
public class MagicSteelTalonFX implements IMotorController {
    ...
    public void setSetpoint(Rotation2d setpoint) {
        switch (configuration.mode) {
            case kFlywheel:
                talonFX.setControl(new MotionMagicVelocityVoltage(setpoint.getRotations()).withEnableFOC(withFOC)
                        .withFeedForward(configuration.kFF));
                break;
            case kServo:
            case kLinear:
                talonFX.setControl(new MotionMagicVoltage(setpoint.getMeasure()).withEnableFOC(withFOC)
                        .withFeedForward(configuration.kFF));
                break;
            default:
                talonFX.setControl(new MotionMagicVoltage(setpoint.getMeasure()).withEnableFOC(withFOC)
    ...
    public void setSetpoint(double setpoint) {
        ...
        switch (configuration.mode) {
            case kFlywheel:
                talonFX.setControl(new MotionMagicVelocityVoltage(setpoint).withEnableFOC(withFOC)
                        .withFeedForward(configuration.kFF));
                break;
            case kServo:

                talonFX.setControl(
                        new MotionMagicVoltage(setpoint).withEnableFOC(withFOC).withFeedForward(configuration.kFF));
